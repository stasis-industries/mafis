...
Uses paams_*_summary.csv columns `solver_step_us_mean` and `ft_mean`.
"""
...
RESULTS = Path("/Users/teddyadmin/Developments/Research-Project/mafis/results")
...
def load():
    frames = []
    for f in [
        "paams_warehouse_single_dock_summary.csv",
        "paams_warehouse_dual_dock_summary.csv",
    ]:
        p = RESULTS / f
        if p.exists():
