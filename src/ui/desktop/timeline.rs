...
pub fn timeline_ui(
    ...
    egui::TopBottomPanel::bottom("footer").show(ctx, |ui| {
        ui.horizontal(|ui| {
            ...
            match state {
                SimState::Idle => {
                    ...
                    ui.add_enabled(false, egui::Button::new(
                        egui::RichText::new("PAUSE").color(btn_dim)));
                    ui.add_enabled(false, egui::Button::new(
                        egui::RichText::new("STEP").color(btn_dim)));
                    ui.add_enabled(false, egui::Button::new(
                        egui::RichText::new("RESET").color(btn_dim)));
                }
                SimState::Loading => {
                    ui.add_enabled(false, egui::Button::new(
                        egui::RichText::new("START").color(btn_dim)));
                    ui.spinner();
                }
                SimState::Running => {
                    ui.add_enabled(false, egui::Button::new(
            ...
            if recorded > 0 {
                let max_recorded = history.snapshots.back()
                    .map(|s| s.tick)
                    .unwrap_or(0);
            ...
            ui.horizontal(|ui| {
                ...
                for ev in &schedule.events {
                    ...
                    ui.colored_label(color, &label)
                        .on_hover_text(format!("{:?} {}", ev.action, if ev.fired { "(fired)" } else { "" }));
                }
            });
        }
    });
