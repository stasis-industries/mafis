...
use super::astar::{ConstraintChecker, SpacetimeGrid, spacetime_astar_fast};
...
impl Token {
    ...
    fn set_path(&mut self, agent: usize, positions: Vec<IVec2>) {
...
impl MasterConstraintIndex {
    ...
    fn reset(&mut self, width: i32, height: i32, max_time: u64) {
    ...
    fn add_path(&mut self, path: &VecDeque<IVec2>, max_time: u64) {
        if path.is_empty() {
            return;
        }
    ...
    fn remove_path(&mut self, path: &VecDeque<IVec2>, max_time: u64) {
        if path.is_empty() {
            return;
        }
...
mod tests {
    ...
    fn master_ci_add_remove_symmetric() {
        ...
        // Vertex at (1,0) time 1 should be blocked
        assert!(mci.is_vertex_blocked(IVec2::new(1, 0), 1));
        // Stay-at-end: (2,0) should be blocked at times 3..10
        assert!(mci.is_vertex_blocked(IVec2::new(2, 0), 5));

        mci.remove_path(&path, 10);

        // After removal, nothing should be blocked
        assert!(!mci.is_vertex_blocked(IVec2::new(1, 0), 1));
        assert!(!mci.is_vertex_blocked(IVec2::new(2, 0), 5));
    }
}
