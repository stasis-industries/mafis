...
fn fault_metrics_clear_resets_to_zero() {
    ...
    h.app
        .world_mut()
        .resource_mut::<crate::analysis::fault_metrics::FaultMetrics>()
        .clear();
    let m = h.metrics();
    assert_eq!(m.mttr, 0.0);
    assert_eq!(m.throughput, 0.0);
...
fn scorecard_clear_resets() {
    ...
    h.app
        .world_mut()
        .resource_mut::<crate::analysis::scorecard::ResilienceScorecard>()
        .clear();
    assert_eq!(h.scorecard().fault_tolerance, 1.0);
...
fn tick_history_clear_empties_snapshots() {
    ...
    h.app
        .world_mut()
        .resource_mut::<crate::analysis::history::TickHistory>()
        .clear();
    assert!(h.history().snapshots.is_empty());
}
...
fn heatmap_density_accumulates_when_visible() {
    ...
    h.app
        .world_mut()
        .resource_mut::<crate::analysis::AnalysisConfig>()
